	...
	<ul id="task-card" class="collection with-header" style="margin-bottom: 0px; border: 0px none;">
		<li class="collection-header" style="background:transparent;padding:0;">
			<div style="z-index: 2147483647; position: fixed; top: 15px; left: calc(100% - 470px);">
				<a data-delay="50" data-tooltip="Next To Pricing" class="btn-floating tooltipped white" style="float: right;margin-left:10px;" href="<?php echo SITE_ROOT."/project/internal_pricing?project_id=".$_REQUEST['project_id']; ?>">
				...
				<a data-delay="50" data-tooltip="Previous To Estimate" class="btn-floating tooltipped white" style="float: right;margin-left:10px;" href="<?php echo SITE_ROOT."/project/internalestimate?project_id=".$_REQUEST['project_id']; ?>">
...
$(document).ready(function() {
    var table = $('#data-table-row-grouping').DataTable({
		...
		"processing": true,
        "serverSide": true,
		"ajax": "<?= BASEPATH."/project/listintproject?project_id=".$_REQUEST['project_id']; ?>",
		columnDefs: [
...
function deleteData(id)
{
	...
	if (r==true)
	{
		window.location.href="<?=SITE_ROOT;?>/project/deleteintproject?id="+id+"&project_id=<?php echo $_REQUEST['project_id']; ?>";
	}
}
$(function(){
 $('#intproject').addClass('active');
 $("#headingSearchtitle").html("Interior Project");
