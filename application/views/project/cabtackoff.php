...
$(function(){
	$('table td').on('change', function(evt, newValue) {
		...
		var sumtime = 0;
		$('#mainTable tr:not(:first-child) td:nth-child(1)').each(function(){
			try{
				if(parseFloat($(this).html()) > 0)
				{
					//$(this).css({"background":"#333"});
					sumtime += parseFloat($(this).html());
					console.log("pppp:::"+parseFloat($(this).html()));
				}
			}catch(err){}
		});	
		$("#hours1").html(sumtime);
		...
		var sumtime2 = 0;
		$('#mainTable tr:not(:first-child) td:nth-child(4)').each(function(){
			try{
				if(parseFloat($(this).html()) > 0)
				{
					//$(this).css({"background":"#333"});
		...
		var sumtime3 = 0;
		$('#mainTable tr:not(:first-child) td:nth-child(7)').each(function(){
			try{
				if(parseFloat($(this).html()) > 0)
				{
					//$(this).css({"background":"#333"});
		...
		var sumtime4 = 0;
		$('#mainTable tr:not(:first-child) td:nth-child(10)').each(function(){
			try{
				if(parseFloat($(this).html()) > 0)
				{
					//$(this).css({"background":"#333"});
		...
		var sumtime5 = 0;
		$('#mainTable tr:not(:first-child) td:nth-child(13)').each(function(){
			try{
				if(parseFloat($(this).html()) > 0)
				{
					//$(this).css({"background":"#333"});
	...
	/*	var sumtime6 = 0;
		...
		$("#TOTALBID").val(parseFloat($("#LaborDollars").val()) + parseFloat($("#MaterialDollars").val()));
	});
});
</script>
<style>
...
<div class="row">
	<div class="col s12 m12">
		...
		<li class="collection-header" style="background:transparent;padding:0;">
			<div style="z-index: 2147483647; position: fixed; top: 15px; left: calc(100% - 530px);">
					...
					<a href="<?php echo SITE_ROOT."/home/externalnote?project_id=".$_REQUEST['project_id']; ?>" style="color: white; font-size: 25px; display:none;" data-tooltip="Create New Note">
						<i class="mdi-content-content-paste"></i>
					</a>
					<a href="<?php echo SITE_ROOT."/project/externalestimate?project_id=".$_REQUEST['project_id']; ?>" style="color: white; font-size: 25px; display:none;" data-tooltip="Create New Estimate">
						<i class="mdi-action-assignment-late"></i>
					</a>
					<a href="<?php echo SITE_ROOT."/project/external_project?project_id=".$_REQUEST['project_id']; ?>" style="color: white; font-size: 25px; display:none;" data-tooltip="Create New Project">
						<i class="mdi-action-assignment"></i>
					</a>
					<a href="<?php echo SITE_ROOT."/project/external_pricing?project_id=".$_REQUEST['project_id']; ?>" style="color: white; font-size: 25px; display:none;" data-tooltip="External Pricing">
						<i class="mdi-editor-attach-money"></i>
					</a>
					<a href="<?php echo SITE_ROOT."/home/notes?project_id=".$_REQUEST['project_id']; ?>" style="color: white; font-size: 25px; display:none;" data-tooltip="Create General Note">
	...
	<div id="row-grouping" class="section" style="padding-top:0px;">
		...
		<div class="row">
			<div class="col s12 m12 tableheight">
				...
				<table class="table table-striped specialclass2" style="border-collapse: separate; border-spacing: 1px ! important; background: #009688;">
					<thead>
							<tr>
								<th>Hours</th>
								<th>Task</th>
...
$(function(){
 ...
 $("#headingSearchtitle").html("Cab Take Off");
});
