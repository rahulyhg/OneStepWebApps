...
class Api extends Controller 
{
	...
	public function createnewproject(){
		...
		else
		{
			...
			{	
				...
				$result66 = $db->Query("INSERT INTO default_production_rate(production_item,description,rate,unit,coat_1,coat_2,coat_3,coat_4,spread,factor,project_id,client_id)SELECT production_item,description,rate,unit,coat_1,coat_2,coat_3,coat_4,spread,factor,'".$result55."','".$user[0]['id']."' FROM default_production_rate WHERE project_id = '0'");
	...
	public function fetchdropdown(){
			...
			if($param=="Country")
				...
				echo $db->CreateOptions("json", "room_types", array("id","name"),"","","");
				...
				echo $db->CreateOptions("json", "ext_room_type", array("id","name"),"","","");
	...
	public function pricingint(){
		...
		$join_tables = array(
			...
			array('left','location r1','r1.location_id = p1.City', array('r1.name as city_name')),
			array('left','location r2','r2.location_id = p1.State', array('r2.name as state_name')),
			array('left','location r3','r3.location_id = p1.country', array('r3.name as country_name'))
		);
	...
	public function listallclients()
  	{
		...
		{	
			//echo "here";
			if(isset($row->city_name)){$city_name = ",".$row->city_name;} else {$city_name = "";}
			if(isset($row->state_name)){$state_name = ",".$row->state_name;} else {$state_name = "";}
			$temp = array();
			$temp['id'] = $row->id;
			...
			$temp['name'] = $row->client_name;
			$temp['Address'] = $row->Address."".$city_name."".$state_name;
